...
const CommunicationList = () => {
  ...
  useEffect(() => {
    const fetchCommunications = async () => {
      try {
        const response = await axios.get(`https://revisionbackend.vercel.app/api/communications`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://revisionbackend.vercel.app/api/communications/${id}`, {
        headers: {
  ...
  return (
    <div className="p-6 bg-customBg min-h-screen flex flex-col justify-center items-center">
      <h1 className="text-3xl font-bold mb-6">Liste des communications</h1>
        ...
        {communications.map((communication) => (
          <li key={communication._id} className="border p-4 mb-4 rounded shadow-lg bg-white text-center">
            ...
            <div className="flex justify-between">
              <Link to={`/navbartwo/edit-communication/${communication._id}`} className="bg-yellow-500 text-white p-2 rounded flex items-center justify-center">
                <Edit size={18} />
              </Link>
              <button onClick={() => handleDelete(communication._id)} className="bg-red-500 text-white p-2 rounded flex items-center justify-center">
                <Trash size={18} />
              </button>
            </div>
          </li>
      ...
      <Link to="/navbartwo/create-communication" className="mt-6 bg-blue-500 text-white p-2 rounded w-40 text-center flex items-center justify-center">
