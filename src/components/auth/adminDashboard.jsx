...
const AdminDashboard = () => {
  ...
  const updateUserRole = async (userId, newRole) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://revisionbackend.vercel.app/api/admin/users/${userId}/role`,
        { role: newRole },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const deleteUser = async (userId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://revisionbackend.vercel.app/api/admin/users/${userId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 md:p-6">
      ...
      {loading && (
        <div className="text-blue-500 mb-4 text-center">Chargement...</div>
      )}
      {message && (
                ...
                <td className="py-2 px-4 border-b">
                  <div className="flex flex-nowrap gap-2 overflow-x-auto">
                    <button
                      ...
                      className="px-3 py-1 bg-blue-500 text-white rounded text-xs md:text-sm whitespace-nowrap"
                    >
                      User
                    </button>
                    <button
                      onClick={() => updateUserRole(user._id, 'agent')}
                      className="px-3 py-1 bg-green-500 text-white rounded text-xs md:text-sm whitespace-nowrap"
                    >
                      Agent
                    </button>
                    <button
                      onClick={() => updateUserRole(user._id, 'admin')}
                      className="px-3 py-1 bg-red-500 text-white rounded text-xs md:text-sm whitespace-nowrap"
                    >
                      Admin
                    </button>
                    <button
                      onClick={() => deleteUser(user._id)}
                      className="px-3 py-1 bg-yellow-600 text-white rounded text-xs md:text-sm whitespace-nowrap"
                    >
                      Supprimer
                    </button>
                  </div>
